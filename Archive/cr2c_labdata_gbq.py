...
def get_data(
	...
	for ltype in ltypes:
		...
		# query_job = client.query("SELECT * FROM `prefab-overview-218500.test_dataset.{0}`".format(ltype))

		# results = query_job.result()  # Waits for job to complete.

		# ldata_long = results.to_dataframe
...
class labrun:
	...
	def process_data(self):
		...
		for ltype in self.ltype_list:
			...
			ldata_long.to_gbq('test_dataset.{0}'.format(ltype),projectid,if_exists='append')
